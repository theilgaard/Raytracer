...
{
public:
    TriangleMesh(ShadingModel* default_material = NULL);
	...
	ShadingModel** materials()  {return m_materials;}
    int numTris()           {return m_numTris;}
	
	void connectNameToMaterial(const char* name, ShadingModel* material);
	void setDefaultMaterial(ShadingModel* m)
	{
		m_defaultMaterial = m;
	}	
	
	ShadingModel* findMaterialByName(const char* const name);
...
protected:
	...
	ShadingModel* m_defaultMaterial;
	std::vector<ShadingModel*> m_materialList;
