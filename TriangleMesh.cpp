...
TriangleMesh::TriangleMesh(ShadingModel* default_material) :
    m_normals(0),
    m_vertices(0),
    m_texCoords(0),
    m_normalIndices(0),
...
void TriangleMesh::createSingleTriangle()
{
	...
	m_materials = new ShadingModel*[1];
...
void TriangleMesh::connectNameToMaterial(const char* name, ShadingModel* material)
{
...
void TriangleMesh::loadObj(FILE* fp, const Matrix4x4& ctm)
{
	...
	m_materials = new ShadingModel*[nf];

	ShadingModel* currentMaterial = m_defaultMaterial;
	m_hasTextureCoords = false;
    ...
    while (fgets(line, 80, fp) != 0)
    {
        ...
        else if (line[0] == 'f')
        {
			...
			{
				fprintf(stderr,"Missing a material for this object. Set default material.\n");
 				exit(1);
			}
...
ShadingModel* TriangleMesh::findMaterialByName(const char* const name)
{
